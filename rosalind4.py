...
plus any new offspring. A key observation is that the number of offspring in any
month is equal to the number of rabbits that were alive two months prior.
...
fibs = {1:1}
def fib(months, offspring=1):
    if months in fibs.keys():
        return fibs[months]
    elif months == 2:
        fibs[months] = offspring + 1
    else:
        fibs[months] = fib(months-1, offspring) + (fib(months-2, offspring) * offspring)
    return fibs[months]
